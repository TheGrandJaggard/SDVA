...
{
    /// <summary>
    /// Allows a UI element to be clicked and dropped to and from a container.
    /// 
    ...
    /// or `IItemSource` and `IItemDestination` to update the interface after a drop
    /// has occurred.
    /// </summary>
    /// <typeparam name="T">The type that represents the item being dragged.</typeparam>
    public class ClickAndDropItem<T> : MoveItem<T>, IPointerClickHandler
        where T : class
    {
        private Vector3 startPosition;
        private Transform originalParent;
        private IItemContainer<T> cursorContainer;
